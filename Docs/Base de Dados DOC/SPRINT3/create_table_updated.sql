...
(
    ...
    facilityType varchar2(32),
    countryId    varchar2(32),
    latitude     number CHECK ( Facility.latitude <= 90 || Facility.latitude >= -90 ),
    longitude    number CHECK ( Facility.latitude <= 180 || Facility.latitude >= -180 ),
    name         varchar(32),
...
(
    ...
    constraint pk_Border primary key (firstCountryID, secondCountryID),
    constraint fk_Border_firstCountryID foreign key (firstCountryID) references COUNTRY,
    constraint fk_Border_secondCountryID foreign key (secondCountryID) references COUNTRY
);
...
(
    ...
    countryID varchar2(32),
    name      varchar2(32),
    constraint fk_City_countryID foreign key (countryID) references COUNTRY
...
(
    ...
    latitude   number CHECK ( Address.latitude <= 90 || Address.latitude >= -90 ),
    longitude  number CHECK ( Address.latitude <= 180 || Address.latitude >= -180 ),
    constraint fk_Address_cityID foreign key (cityId) references CITY

);

alter table CLIENT
    add addressID varchar2(32);

alter table CLIENT
    add constraint fk_Client_addressID foreign key (addressID) references ADDRESS;
